                ...
                return new Promise(function(resolve, reject) {
                    ...
                    xhr.onload = function() {
                        if (xhr instanceof XMLHttpRequest) {
                            if (xhr.status === 200) {
                                if (responseType === 'text') {
                                    ...
                                    reader.addEventListener('load', function() {
                                        this.cache[src] = Promise.resolve(reader.result);
                            ...
                            this.cache[src] = Promise.resolve(xhr.responseText);
        ...
        /***/ (function(module, exports, __webpack_require__) {
                            ...
                            value: function inlineFonts(document) {
                                ...
                                return Promise.all(Array.from(document.styleSheets).map(function(sheet) {
                                    ...
                                    return Promise.all(fonts.map(function(font) {
                                        const fontSrc = font.formats[0].src;
                                        if (_this2.hasResourceInCache(fontSrc)) {
                                            return _this2.cache[fontSrc];
                                        }
                                        return fetch(fontSrc).then(function(response) {
                                            ...
                                            return new Promise(function(resolve, reject) {
                                                ...
                                                reader.onload = function() {
                                                    ...
                                                    _this2.cache[fontSrc] = Promise.resolve(result);
                                                    resolve(result);
                                                };
                        ...
                        },
                        {
                            ...
                            value: function createElementClone(node) {
                                ...
                                if (node instanceof HTMLStyleElement && node.sheet && node.sheet.cssRules) {
                                    var cssRules = (node.sheet.cssRules ? Array.from(node.sheet.cssRules) : []).filter(function(rule) {
                                        // 在打开截图插件时，有可能造成部分的rule没有style的情况。需要排除
                                        if (!rule.style) {
                                            return false;
                                        }
                                        const propertyValue = rule.style.getPropertyValue('font-family');
                                        const includeFont = _this3.options.fonts && _this3.options.fonts.length > 0 ? _this3.options.fonts.includes(propertyValue) : false;
                                        return rule.type === CSSRule.FONT_FACE_RULE && includeFont;
                                    });
                                    var css = [].slice.call(cssRules, 0).reduce(function(css, rule) {
                                        try {
                            ...
                            value: function cloneNode(node) {
                                ...
                                for (var child = node.firstChild; child; child = child.nextSibling) {
                                    if (child.nodeType !== Node.ELEMENT_NODE || child.nodeName !== 'SCRIPT' &&
                                        // $FlowFixMe
                                        (!child.hasAttribute(IGNORE_ATTRIBUTE) || child.getAttribute(IGNORE_ATTRIBUTE) === this.options.fontFamily) &&
            ...
            var getSheetFonts = function getSheetFonts(sheet, document) {
                ...
                return (sheet.cssRules ? Array.from(sheet.cssRules) : []).filter(function(rule) {
                    const propertyValue = rule.style.getPropertyValue('font-family');
                    const includeFont = this.options.fonts && this.options.fonts.length > 0 ? this.options.fonts.includes(propertyValue) : false;
                    return rule.type === CSSRule.FONT_FACE_RULE && includeFont;
                }).map(function(rule) {
            ...
            var asArray = function asArray(arrayLike) {
                var array = [];
                var length = arrayLike.length;
                for (var i = 0; i < length; i++) array.push(arrayLike[i]);
                return array;
            };
            var removeRule = function removeRule(styleSheets) {
                for (let i = styleSheets.length - 1; i >= 0; i--) {
                    try {
                        const cssRules = styleSheets[i].cssRules || styleSheets[i].rules;
                        if (cssRules && cssRules.length > 0) {
                            for (let j = cssRules.length - 1; j >= 0; j--) {
                                if (cssRules[j] && cssRules[j].type !== 1) {
                                    if (typeof cssRules.deleteRule != 'undefined') {
                                        cssRules.deleteRule(j);
                                    } else if (typeof cssRules.removeRule != 'undefined') {
                                        cssRules.removeRule(j);
                                    }
                                }
                            }
                        }
