...
import { CANVAS_TYPE, LABEL_LIST, SEGMENT_TYPE, VIDEO_TYPE } from '../../../../config/staticParams';
...
export default class MouldMaterial extends React.Component {
    ...
    componentDidUpdate(prevProps, prevState) {
        const { fragment, classify, style } = this.state;
    ...
    loadList = (page = 1) => {
        ...
        const { list, classify, style, fragment, orderBy } = this.state;
        ...
        const params = {
            ...
            videoDuration: '',
            key: '',
            templateTypes: VIDEO_TYPE.realShoot,
        };
        getIndex(params)
            .then(res => {
                const { data } = res;
    ...
    onUserTemplate = (id = null) => {
        this.props.dispatch({
            type: 'workspace/insertVideo',
            payload: {
                id: id || this.state.hoverObj.id,
    ...
    render() {
        const { state: { ...state }, props: { scrolling } } = this;
                ...
                <React.Fragment>
                    <div className={styles.list}>
                        <div className={`${styles.videoMart} ${scrolling ? 'scrollShadow' : ''}`}>
                            <Labels width={60} typeData={[LABEL_LIST.实拍素材内容]}
                                    ...
                                    {state.list.length > 0 &&
                                        state.list.map((item, index) =>
                                            <Card {...item}
                                                direction={state.direction}
                                                key={`${index}-${item.id}`}
                                                onChange={() => this.onUserTemplate(item.id)}
